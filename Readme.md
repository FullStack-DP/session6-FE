# Frontend Session 6


> [!IMPORTANT]  
> - [Homework before frontend-session 7](./material/Homework.md)
> - [Zoom Recordings](https://metropoliafi-my.sharepoint.com/:f:/g/personal/samiben_metropolia_fi/EramjTltA_BOni_R40opXRQBTQ1_gK6EWP2cbcX5G63ghA)

------

### How You'll Learn in This Course

We'll follow a 5-step learning cycle as shown in the figure below:

![](./material/img/learning-phases.png)

-----

### Session Timeline 

- Morning:
  - Mini lecture (~35min)
  - `Break` (~15min)
  - Pair programming (~35min)
  - Pair programming (~35min)
- Lunch Break - 12:00 -13:00
- Afternoon
  - Mini lecture (~35min)
  - `Break` (~15min)
  - Group Activity (~35min)
  - Mini lecture (~35min)
  - Group Activity (~35min)

-----

### Topics

- API Testing
- `Mini Project`

<!-- - [Screenshots](./material/Screenshots.md) -->

-----

### Morning: Pair Programming

<!-- - [Summary](./material/fe-pp-summary.md) -->
- [Level 5 (Advanced)](./material/fe-pp-l5.md) 
- [Level 1 (Beginner)](./material/fe-pp-l1.md)

-----

### Afternoon

<!-- - [Student’s Contribution to Industry 5.0](./material/industry50.md) -->
- ~~Draw.io~~
- You'll learn how to write and run tests for your API to ensure that routes, authentication, and data handling function as expected.
  - Theory related
    - [Summary](./material/be-summary.md)
    - Separation of Server Logic: [app.js](./material/src/app.js) vs [index.js](./material/src/index.js)
    - [cross-env](./material/src/package.json)
  - [Activity 1](./material/be-activity1.md)
  - [Activity 2](./material/be-activity2.md)
  - [Proof of concept: Using Fetch with Jest for Backend Tests](./material/fetch-vs-supertest-poc.md)

----
<details>
<summary>Links used in the Lecture</summary>

### [API Workflow](./material/api-workflow.md)

[![](./material/img/data-flow-testing.png)](./material/api-workflow.md) 

- [Supertest](https://github.com/visionmedia/supertest) 
- [Mocha](https://mochajs.org/) 
- [Chai]( https://www.chaijs.com/)
- [Jest](https://jestjs.io/docs/getting-started) 
- [newman](https://github.com/postmanlabs/newman)  
- [superagent](https://github.com/ladjs/superagent)
- [The Difference Between TDD and BDD]( https://joshldavis.com/2013/05/27/difference-between-tdd-and-bdd/)
- [Run postman collection with newton](https://github.com/postmanlabs/newman#using-newman-cli)
- [curl withh Airport Gap API](https://airportgap.com/docs)
- [How to run, ignore or skip Jest tests, suites and files](https://codewithhugo.com/run-skip-single-jest-test/) 

</details>

  

<!-- links -->



<!-- 

> [!NOTE]  
> Highlights information that users should take into account, even when skimming.

> [!TIP]
> Optional information to help a user be more successful.

> [!IMPORTANT]  
> Crucial information necessary for users to succeed.

> [!WARNING]  
> Critical content demanding immediate user attention due to potential risks.

> [!CAUTION]
> Negative potential consequences of an action. 

-->